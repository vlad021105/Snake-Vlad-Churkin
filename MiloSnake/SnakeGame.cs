        ...
        private void Die()
        {
            ...
            if (gameScore != 0)
            {
                ...
                try
                {
                    ...
                    m_sqlCmd.CommandText = "CREATE TABLE IF NOT EXISTS Record (id INTEGER PRIMARY KEY AUTOINCREMENT, author TEXT, score INTEGER)";
                ...
                try
                {
                    m_sqlCmd.CommandText = "INSERT INTO Record ('author', 'score') values ('" + nickname + "' , '" + gameScore + "')";

                    m_sqlCmd.ExecuteNonQuery();

                    MessageBox.Show("Рекорд записан!");
                }
                catch (SQLiteException ex)
                {
                    MessageBox.Show("Error: " + ex.Message);
